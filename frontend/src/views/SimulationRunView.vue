...
// Read maxRounds from query params on init so the child component receives it immediately
...
const handleGoBack = async () => {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      // Env not running; stop the process if needed
...
const handleNextStep = () => {
  // Step3Simulation The component will directly handle report generation and routing jumps
...
const loadGraph = async (graphId) => {
  // During simulation, avoid full-screen loading on auto refresh to prevent flicker
...
onMounted(() => {
  ...
  // Document the maxRounds configuration (the value has been obtained from the query parameter on initialization)
  if (maxRounds.value) {
