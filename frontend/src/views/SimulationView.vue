...
const handleNextStep = (params = {}) => {
  ...
  // Pass custom max rounds through query params
  if (params.maxRounds) {
...
/**
 * Checks and closes a running simulation.
 * When the user goes back from Step 3 to Step 2, assume they intend to exit the simulation.
 */
const checkAndStopRunningSimulation = async () => {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      // Env not running, but the process might still be running
    ...
    // Status check failure should not block the flow
    console.warn('Failed to check simulation status:', err)
  }
}

/**
 * Force-stops the simulation
 */
...
onMounted(async () => {
  ...
  // Check and close any running simulation (when returning from Step 3)
