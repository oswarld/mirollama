          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- CREATE_COMMENT: Leave a comment -->
                ...
                <!-- Universal fallback: unknown type or content but not handled above -->
...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // The maximum number of rounds passed in from Step2
  minutesPerRound: {
    type: Number,
    default: 30 // Default is 30 minutes per round
  },
  projectData: Object,
  graphData: Object,
  systemLogs: Array
...
const actionIds = ref(new Set()) // Action ID collection used for deduplication
const scrollContainer = ref(null)

// Computed
// Display actions in chronological order (newest at the end, i.e. at the bottom)
...
// Format simulation elapsed time (calculated in terms of rounds and minutes per round)
...
// TwitterSimulated elapsed time for the platform
const twitterElapsedTime = computed(() => {
  return formatElapsedTime(runStatus.value.twitter_current_round || 0)
})

// RedditSimulated elapsed time for the platform
...
const resetAllState = () => {
  ...
  stopPolling()  // Stop polling that may have existed before
}
...
const doStartSimulation = async () => {
  ...
  // Reset all states first to ensure they are not affected by the last simulation
  resetAllState()
  ...
  try {
    const params = {
      ...
      enable_graph_memory_update: true  // Enable dynamic map updates
    }
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect round changes on each platform separately and output logs
      ...
      // Detect whether the simulation has been completed (judged by runner_status or platform completion status)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Additional check: If the backend has not had time to update runner_status, but the platform has reported completion
      // Judgment by detecting twitter_completed and reddit_completed status
...
const checkPlatformsCompleted = (data) => {
  ...
  // Use actions_count to determine whether the platform is enabled (if count > 0 or running was true)
  ...
  // Check that all enabled platforms have been completed
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      // Use all_actions to get a complete list of actions
      const serverActions = res.data.all_actions || []
      
      // Incrementally add new actions (remove duplication)
      ...
      // No automatic scrolling, allowing users to view the timeline freely
      // New actions will be added at the bottom
    }
  } catch (err) {
