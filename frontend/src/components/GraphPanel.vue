  ...
  <div class="graph-panel">
    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <!-- Under construction/Prompt during simulation -->
...
const expandedSelfLoops = ref(new Set()) // expanded self-loop term
const showSimulationFinishedHint = ref(false) // Prompt after simulation ends
const wasSimulating = ref(false) // Tracking whether it was previously in simulation

// Close simulation end prompt
...
// Toggle self-loop item expansion/folded state
...
const renderGraph = () => {
  ...
  tempEdges.forEach(e => {
    ...
    // Determine whether the direction of the edge is consistent with the normalized direction (sourceUUID < target UUID)
  ...
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      ...
      // Basic distance 150, increasing for each additional side 40
