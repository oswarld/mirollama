  ...
  >
    ...
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        ...
        <!-- Bottom decorative line (hovertime to expand) -->
...
const containerStyle = computed(() => {
  ...
  // Calculate actual required height: number of rows * card height + (Number of lines-1) * spacing + small amount of bottom spacing
...
// Format time (when displayed:point)
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Shorter delay when expanding(50ms), the delay is longer when folding(200ms)to increase stability
...
.cards-container {
  ...
  /* min-height Dynamically calculated by JS, adaptive according to the number of cards */
}
