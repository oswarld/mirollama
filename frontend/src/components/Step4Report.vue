            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (Content generation is completed, but the entire chapter may not be completed yet) -->
                  ...
                  <!-- Section Complete (Chapter generation completed) -->
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- When the answer is final, a special prompt is displayed -->
...
const toolConfig = {
  'insight_forge': {
    name: 'Deep Insight',
    color: 'purple',
    icon: 'lightbulb' // Light bulb icon - represents insight
  },
  'panorama_search': {
  ...
  'interview_agents': {
    ...
    color: 'green',
    icon: 'users' // User icon - represents a conversation
  },
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
...
const PLATFORM_NO_RESPONSE_TEXTS = new Set([
  '(The platform did not receive a reply)',
  '(The platform did not receive a reply)',
  '[no reply]',
...
const parseInsightForge = (text) => {
  ...
  try {
    const queryMatch = text.match(/(?:Analyze the problem|Question)\s*:\s*(.+?)(?:\n|$)/i)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    const reqMatch = text.match(/(?:Prediction scenarios|Scenario|Simulation requirement)\s*:\s*(.+?)(?:\n|$)/i)
    if (reqMatch) result.simulationRequirement = reqMatch[1].trim()
    
    const factMatch = text.match(/(?:Relevant prediction facts|Related facts)\s*:\s*(\d+)/i)
    const entityMatch = text.match(/(?:Involved entities|Entities)\s*:\s*(\d+)/i)
    const relMatch = text.match(/(?:relationship chain|Relationship chains)\s*:\s*(\d+)/i)
    if (factMatch) result.stats.facts = parseInt(factMatch[1])
    if (entityMatch) result.stats.entities = parseInt(entityMatch[1])
    if (relMatch) result.stats.relationships = parseInt(relMatch[1])
    
    const subQSection = text.match(/###\s*(?:Analytical sub-problems|Sub-questions)\n([\s\S]*?)(?=\n###|$)/i)
    if (subQSection) {
      const lines = subQSection[1].split('\n').filter(l => l.match(/^\d+\./))
      result.subQueries = lines.map(l => l.replace(/^\d+\.\s*/, '').trim()).filter(Boolean)
    }
    
    const factsSection = text.match(/###\s*(?:[Key facts]|Key Facts)[\s\S]*?\n([\s\S]*?)(?=\n###|$)/i)
    if (factsSection) {
    ...
    const entitySection = text.match(/###\s*(?:【Core entity】|Core Entities)\n([\s\S]*?)(?=\n###|$)/i)
    if (entitySection) {
      ...
      result.entities = entityBlocks.map(block => {
        ...
        const summaryMatch = block.match(/(?:summary|Summary)\s*:\s*"?(.+?)"?(?:\n|$)/i)
        const relatedMatch = block.match(/(?:Relevant facts|Related facts)\s*:\s*(\d+)/i)
        return {
    ...
    const relSection = text.match(/###\s*(?:【Relationship chain】|Relationship chains)\n([\s\S]*?)(?=\n###|$)/i)
    if (relSection) {
...
const parsePanorama = (text) => {
  ...
  try {
    const queryMatch = text.match(/(?:Query|Query)\s*:\s*(.+?)(?:\n|$)/i)
    if (queryMatch) result.query = queryMatch[1].trim()
    ...
    const histMatch = text.match(/history\/expired fact:\s*(\d+)/)
    ...
    // Extract history/Expired Facts - Full withdrawal, no limit on quantity
...
const parseInterview = (text) => {
  ...
  try {
    const isNoResponseText = (value) => {
      if (!value) return true
      return PLATFORM_NO_RESPONSE_TEXTS.has(value.trim())
    }

    const topicMatch = text.match(/\*\*(?:Interview topics|Interview topic)\s*:\*\*\s*(.+?)(?:\n|$)/i)
    if (topicMatch) result.topic = topicMatch[1].trim()
    
    const countMatch = text.match(/\*\*(?:Number of interviews|Interview count)\s*:\*\*\s*(\d+)\s*\/\s*(\d+)/i)
    if (countMatch) {
    ...
    const reasonMatch = text.match(/###\s*(?:Reasons for selecting interviewees|Selection reason)\n([\s\S]*?)(?=\n---\n|\n###\s*(?:Interview transcript|Interviews))/i)
    if (reasonMatch) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Format2: - Choosing a name (index X): reason
        // For example: - Choose Parents_601 (index 0): As a representative of the parent group...
        if (!headerMatch) {
        ...
        // For example: - **Parents_601 (index 0)**: As a representative of the parent group...
        if (!headerMatch) {
        ...
        if (name) {
          ...
          // continuation of reasons (excluding concluding paragraphs)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Extract the title (such as "student","education practitioner" wait)
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // If the first question is preceded by"1.", requiring special treatment
    ...
    // Extract answers (Twitter/Reddit)
      const answerMatch = block.match(/\*\*A:\*\*\s*([\s\S]*?)(?=\*\*Key Quotes|$)/)
      if (answerMatch) {
        const answerText = answerMatch[1].trim()
        
      const twitterMatch = answerText.match(/(?:[Twitter platform answer]|\[Twitter Answer\])\n?([\s\S]*?)(?=(?:【Reddit platform answer】|\[Reddit Answer\])|$)/i)
      const redditMatch = answerText.match(/(?:【Reddit platform answer】|\[Reddit Answer\])\n?([\s\S]*?)$/i)
        ...
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer, only copied to display by default when not placeholder text
        if (!isNoResponseText(interview.redditAnswer)) {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
        if (!isNoResponseText(interview.twitterAnswer)) {
            interview.redditAnswer = interview.twitterAnswer
          }
        } else if (!twitterMatch && !redditMatch) {
          // No sub-platform mark (very old format), as a whole answer
    ...
    const summaryMatch = text.match(/###\s*(?:Interview summary and key points|Interview summary)\n([\s\S]*?)$/i)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    const queryMatch = text.match(/(?:search query|Search query)\s*:\s*(.+?)(?:\n|$)/i)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    const countMatch = text.match(/(?:turn up|Found)\s*(\d+)\s*(?:strip|results?)/i)
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const isPlaceholderText = (text) => {
      if (!text) return true
      return PLATFORM_NO_RESPONSE_TEXTS.has(text.trim())
    }
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // Supported formats:
      // 1) "Question X:" / "questionX:" or "Q1:" / "Question 1:"
      // 2) "1. " or "\n1. " (legacy numeric format)
      let matches = []
      let match

      // Try first "Question X:" Format
      const cnPattern = /(?:^|[\r\n]+)question(\d+)[::]\s*/g
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Splitting failed: first question returns full answer, rest returns empty
    ...
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Both platforms have real answers (not placeholder text) and different content
...
const renderMarkdown = (content) => {
  ...
  // Remove the secondary title at the beginning (## xxx), because the chapter title has been displayed in the outer layer
  ...
  // Processing lists - supports sublists
  ...
  // Handling line breaks - blank lines become paragraph breaks, single line breaks become <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // clean up <p><br> Following block-level elements (caused by extra blank lines)
  ...
  // Fix numbering of non-consecutive ordered lists: when single item <ol> Keep numbering incremented when separated by paragraph content
...
const getLogLevelClass = (log) => {
  ...
  // INFO Use default colors and don't mark them as success
  return ''
}
...
const extractFinalContent = (response) => {
  ...
  // try to find Final Answer: The following content (supports multiple formats)
  ...
  // Try to find the final answer: What follows
  const chineseFinalMatch = response.match(/final answer[::]\s*\n*([\s\S]*)$/i)
  if (chineseFinalMatch) {
    return chineseFinalMatch[1].trim()
  }
  
  // If with ## or # or > At the beginning, it may be direct markdown content
