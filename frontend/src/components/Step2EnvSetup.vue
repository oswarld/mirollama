<template>
  <div class="env-setup-panel">
    <div class="scroll-container">
      ...
      <!-- Step 03: Generate a dual-platform simulation configuration -->
...
// State
const phase = ref(0) // 0: initialization, 1: Generate character, 2: Generate configuration, 3: Finish
...
const getStageLabel = (stageCode, stageName) => {
  if (stageCode && STAGE_LABELS[stageCode]) return STAGE_LABELS[stageCode]
...
const autoGeneratedRounds = computed(() => {
  if (!simulationConfig.value?.time_config) {
    return null // Returned when the configuration is not generated null
  }
  const totalHours = simulationConfig.value.time_config.total_simulation_hours
  const minutesPerRound = simulationConfig.value.time_config.minutes_per_round
  if (!totalHours || !minutesPerRound) {
    return null // Returned when configuration data is incomplete null
  }
...
// Get the corresponding one based on agent_idusername
...
const startPrepareSimulation = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      startPolling()
      // Start real-time acquisition Profiles
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.progress_detail) {
        ...
        const stageLabel = getStageLabel(detail.current_stage, detail.current_stage_name)
