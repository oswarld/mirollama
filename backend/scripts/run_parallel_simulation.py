...
if sys.platform == 'win32':
    # Set Python default I/O encoded as UTF-8
    ...
    # Force a default encoding (affects open() the default encoding of the function)
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None, 
        ...
        """
        Package open() function, which uses UTF-8 encoding by default for text mode
...
def disable_oasis_logging():
    ...
    OASIS's logs are too redundant (recording the observations and actions of each agent), we use our own action_logger
    """
...
# TwitterAvailable actions (excluding INTERVIEW, INTERVIEW can only be triggered manually through ManualAction)
TWITTER_ACTIONS = [
...
# RedditAvailable actions (excluding INTERVIEW, INTERVIEW can only be triggered manually through ManualAction)
REDDIT_ACTIONS = [
    ActionType.LIKE_POST,
...
class ParallelIPCHandler:
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        if success_count > 0:
            ...
            print(f"  InterviewFinish: agent_id={agent_id}, Number of successful platforms={success_count}/{len(platforms_to_interview)}")
            return True
        else:
            errors = [f"{p}: {r.get('error', 'unknown error')}" for p, r in results["platforms"].items()]
            self.send_response(command_id, "failed", error="; ".join(errors))
            print(f"  Interviewfail: agent_id={agent_id}, All platforms fail")
            return False
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        if twitter_interviews and self.twitter_env:
            try:
                ...
                print(f"  TwitterBatch interviews failed: {e}")
        ...
        if reddit_interviews and self.reddit_env:
            try:
                ...
                print(f"  RedditBatch interviews failed: {e}")
        ...
        if results:
            ...
            print(f"  Batch interviews completed: {len(results)} indivualAgent")
            return True
        else:
    ...
    async def process_commands(self) -> bool:
        ...
        Returns:
            True means to continue running, False means that it should exit
        """
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    """
    Get from simulation_config agent_id -> entity_name mapping
    
    This shows the real entity name in actions.jsonl instead of "Agent_0" Such a code name
...
def fetch_new_actions_from_db(
    ...
    Args:
        ...
        last_rowid: The maximum rowid value last read (use rowid instead of created_at because created_at has different formats on different platforms)
        agent_names: agent_id -> agent_name mapping
        
    Returns:
        (actions_list, new_last_rowid)
        - actions_list: Action list, each element contains agent_id, agent_name, action_type, action_args(with contextual information)
...
def _enrich_action_context(
    ...
    try:
        ...
        # Follow users: Supplement the name of the followed user
            ...
            if follow_id:
                # Get from follow table followee_id
                cursor.execute("""
        ...
        # Blocked user: Supplement the name of the blocked user
            ...
            # Get user_id from action_args or target_id
        ...
        # Post a comment: Supplement the information of the commented post
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    if not os.environ.get("OPENAI_API_KEY"):
        raise ValueError("API Key configuration is missing, please set it in the .env file in the project root directory LLM_API_KEY")
...
async def run_reddit_simulation(
    ...
    # Reddit Use accelerated LLM configuration (if available, otherwise fall back to generic configuration)
...
async def main():
    parser = argparse.ArgumentParser(description='OASISDual platform parallel simulation')
    parser.add_argument(
        '--config', 
        type=str, 
        required=True,
        ...
        try:
            while not _shutdown_event.is_set():
                ...
                if not should_continue:
                    break
                # Use wait_for instead of sleep so it can be responsive shutdown_event
                try:
...
def setup_signal_handlers(loop=None):
    """
    Set up the signal handler to ensure reception SIGTERM/SIGINT able to exit correctly
    ...
    def signal_handler(signum, frame):
        ...
        # Don't be direct sys.exit(), let the asyncio loop exit normally and clean up resources
