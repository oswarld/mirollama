...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        if not os.path.exists(config_path):
            raise ValueError("Simulation config not found. Please run /prepare first.")
        ...
        try:
            ...
            # Create a master log file to avoid stdout/stderr Pipe buffer full causing process blocking
            ...
            # use start_new_session=True Create a new process group and ensure that all child processes can be killed via os.killpg
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        # Returns if the platform is enabled but not completed False
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        ...
        Args:
            ...
            simulation_id: Impersonation ID (for logging)
            timeout: Timeout to wait for the process to exit (seconds)
        """
        if IS_WINDOWS:
            # Windows: Terminate the process tree using the taskkill command
            ...
            # due to using start_new_session=True, the process group ID is equal to the main process PID
            ...
            try:
                ...
                # If it does not end after the timeout, force sending SIGKILL
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        ...
        - twitter_simulation.db(simulated database)
            ...
            try:
                if process.poll() is None:  # Process is still running
                    ...
                    if state:
                        ...
                        state.reddit_running = False
                        state.completed_at = datetime.now().isoformat()
                        state.error = "Simulation terminated due to server shutdown."
                        cls._save_run_state(state)
                    
                    # Also update state.json and set the state to stopped
                    try:
                        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
    ...
    def register_cleanup(cls):
        ...
        # Flask debug In mode, only register cleanup in the reloader child process (the process that actually runs the application)
        # WERKZEUG_RUN_MAIN=true Represents a reloader child process
        ...
        def cleanup_handler(signum=None, frame=None):
            """Signal handler: cleanup simulations first, then call original handler."""
        ...
        try:
            ...
            # SIGHUP: Terminal closes (Unix systems only)
            if has_sighup:
    ...
    def check_env_alive(cls, simulation_id: str) -> bool:
        ...
        Returns:
            True Indicates that the environment is alive, False indicates that the environment is shut down
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
            return False
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specify platform (optional)
                ...
                - None: During dual-platform simulation, interview two platforms at the same time and return the integrated results.
        ...
        if not ipc_client.check_env_alive():
            raise ValueError(f"Simulation environment is not running. Interview is unavailable: {simulation_id}")
    ...
    def interview_agents_batch(
        ...
        """
        Interview multiple batchesAgent
        ...
        Args:
            ...
            platform: Default platform (optional, will be overridden by the platform of each interview item)
                - "twitter": By default, only the Twitter platform is interviewed
                - "reddit": By default, only the Reddit platform is interviewed
    ...
    def interview_all_agents(
        ...
        Use the same questions to interview all in the simulationAgent
    ...
    def close_simulation_env(
        ...
        try:
            ...
            return {
                "success": True,
                "message": "Close-environment command sent (timeout while waiting; environment may still be shutting down)."
            }
    ...
    def get_interview_history(
        ...
        Args:
            ...
            agent_id: Specify Agent ID (optional, only get the history of this Agent)
