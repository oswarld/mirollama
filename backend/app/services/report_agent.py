"""
Report Agentservice
UseLangChain Implement simulation report generation in ReACT mode

Function:
1. Generate reports based on simulation requirements and map information
...
logger = get_logger('mirollama.report_agent')
...
class ReportConsoleLogger:
    ...
    def _setup_file_handler(self):
        ...
        loggers_to_attach = [
            'mirollama.report_agent',
            'mirollama.search_tools',
        ]
...
- History/Expired facts (evolution records)
...
- If you want to make the report more vivid, include"Interview transcript"
...
PLAN_SYSTEM_PROMPT = """\
You are an expert in writing "future forecast reports" and have a "God's perspective" on the simulated world.——You can gain insight into the actions, speech, and interactions of every agent in the simulation.

Core concept:
We build a simulation world and inject specific "simulation requirements" as variables into it. The evolution of the simulated world is a prediction of what may happen in the future. What you are observing is not"experimental data", but"Preview of the future".
...
[Report positioning]
- ✅ This is a simulation-based future forecast report revealing"If so, what will happen in the future?"
- ✅ Focus on predicted results: event trends, group reactions, emerging phenomena, potential risks
...
Forecast scenarios (simulated demand): {simulation_requirement}
...
═══════════════════════════════════════════════════════════════
The most important rules (must be followed):
═══════════════════════════════════════════════════════════════
...
3. [Language consistency - citations must be translated into the reporting language]
   ...
   - This rule applies to both text and quotation blocks (> format)
...
- ❌ It is prohibited to use any Markdown headings within chapters (#,##,###,#### wait)
- ❌ It is forbidden to add the main chapter title at the beginning of the content
- ✅ Chapter titles are automatically added by the system, and you only need to write pure text content
- ✅ use**Bold**, paragraph breaks, quotes, lists to organize content, but no headings

[Correct example]
```
This chapter analyzes the public opinion communication trend of the incident. Through in-depth analysis of simulated data, we found that...
...
### 1. First launch stage     ← mistake! Don't use###divided into sections
...
- interview_agents: Interview simulated agents to obtain the first-person perspectives and real reactions of different characters
...
When you have obtained enough information through the tool to "Final Answer:" Output the chapter content at the beginning.
...
3. Use Markdown format (but prohibit the use of titles):
   - use **bold text** Mark important points (instead of subheadings)
...
4. [Citation format specifications - must be separated into separate paragraphs]
   ...
   ❌ Error format:
   ```
   The school's response was deemed to lack substance.> "The school’s response model..." This evaluation reflects...
   ```
...
7. [Again] Don’t add any titles! use**Bold**Replaces section title"""
...
- ❌ Do not write any title (#,##,###,####None of them work)
- ❌ don't write"{section_title}"as a beginning
- ✅ Chapter titles are automatically added by the system
- ✅ To write the text directly, use**Bold**Replaces section title
...
- If the information is sufficient: with "Final Answer:" Output the chapter content at the beginning (must quote the above original text)
- If more information is needed: call a tool to continue the search
═══════════════════════════════════════════════════════════════"""

REACT_INSUFFICIENT_TOOLS_MSG = (
    "[Note] You only called {tool_calls_count} tools. You must call tools at least {min_tool_calls} times. "
    "Please call a tool again to obtain more simulation data, then output the Final Answer.{unused_hint}"
)

REACT_INSUFFICIENT_TOOLS_MSG_ALT = (
    "Currently only called {tool_calls_count} tools, which require at least {min_tool_calls} Second-rate."
    "Please call the tool to obtain simulation data.{unused_hint}"
)

REACT_TOOL_LIMIT_MSG = (
    "The number of tool calls has reached the upper limit ({tool_calls_count}/{max_tool_calls}), the tool can no longer be called."
    'Please immediately use the information you have obtained to "Final Answer:" Output the chapter content at the beginning.'
)

REACT_UNUSED_TOOLS_HINT = "\n💡 You haven't used it yet: {unused_list}, it is recommended to try different tools to obtain multi-angle information"

REACT_FORCE_FINAL_MSG = "The tool calling limit has been reached, please output directly Final Answer: and generate chapter content."
...
class ReportAgent:
    ...
    def __init__(
        ...
        search_tools: Optional[SearchToolsService] = None
    ):
        """
        ...
        Args:
            ...
            search_tools: SearchTool services (optional)
        """
        ...
        self.search_tools = search_tools or SearchToolsService()
    ...
    def _execute_tool(self, tool_name: str, parameters: Dict[str, Any], report_context: str = "") -> str:
        ...
        try:
            if tool_name == "insight_forge":
                ...
                result = self.search_tools.insight_forge(
                    graph_id=self.graph_id,
                    query=query,
                    simulation_requirement=self.simulation_requirement,
                    report_context=ctx
                ...
                result = self.search_tools.panorama_search(
                    graph_id=self.graph_id,
                    query=query,
                ...
                if isinstance(limit, str):
                    limit = int(limit)
                result = self.search_tools.quick_search(
                    graph_id=self.graph_id,
                    query=query,
                    limit=limit
                )
                ...
                result = self.search_tools.interview_agents(
                    simulation_id=self.simulation_id,
            ...
            # ========== Backward compatibility with old tools (internal redirect to new tools) ==========
                ...
                result = self.search_tools.get_graph_statistics(self.graph_id)
                return json.dumps(result, ensure_ascii=False, indent=2)
            
            elif tool_name == "get_entity_summary":
                entity_name = parameters.get("entity_name", "")
                result = self.search_tools.get_entity_summary(
                    graph_id=self.graph_id,
                ...
                nodes = self.search_tools.get_entities_by_type(
                    graph_id=self.graph_id,
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Format2: Keep it secret - LLM directly outputs bare JSON (without packages) <tool_call> Label)
        # Only try when format 1 is not matched to avoid mismatching in the text. JSON
    ...
    def plan_outline(
        ...
        context = self.search_tools.get_simulation_context(
            graph_id=self.graph_id,
            simulation_requirement=self.simulation_requirement
        )
    ...
    def _generate_section_react(
        ...
        for iteration in range(max_iterations):
            ...
            # ── Conflict handling: LLM outputs both tool calls and Final Answer ──
            ...
            if has_tool_calls:
                # Tool quota has been exhausted → Clearly inform and require output Final Answer
        ...
        # Check whether the LLM return when forced closing is None
        if response is None:
...
class ReportManager:
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        2. put all ### Titles at and below levels are converted to bold text
        ...
        for i, line in enumerate(lines):
            ...
            if heading_match:
                ...
                # Replace all levels of headings (#, ##, ###, ####etc.) converted to bold
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        2. Keep the main title of the report(#)and chapter titles(##), remove other level headers(###, ####wait)
            ...
            if heading_match:
                ...
                if level == 1:
                    if title == outline.title:
                        ...
                        # Chapter title used incorrectly#, revised to##
