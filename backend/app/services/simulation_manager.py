...
logger = get_logger('mirollama.simulation')
...
class SimulationManager:
    ...
    def _build_local_fallback_entities(
        ...
        context_text = (simulation_requirement or document_text or '').strip()
        summary = context_text[:280] if context_text else 'Local fallback entity for offline simulation.'

        entities: List[EntityNode] = []
        for idx, entity_type in enumerate(entity_types, start=1):
            entities.append(
                EntityNode(
                    uuid=f'local-{idx}',
                    name=f'Local{entity_type}{idx}',
    ...
    def prepare_simulation(
        ...
        Args:
            ...
            simulation_requirement: Simulation requirement description (for LLM generation configuration)
            document_text: Original document content (for LLM understanding background)
        ...
        try:
            ...
            if filtered.filtered_count == 0:
                ...
                state.error = "No entities available for simulation preparation."
                self._save_simulation_state(state)
                return state
            ...
            # Reddit It has been saved in real time during the generation process. Save it again here to ensure completeness.
            if progress_callback:
                progress_callback(
            ...
            if state.enable_twitter:
                # TwitterUse CSV format! This is a requirement of OASIS
                generator.save_profiles(
                    profiles=profiles,
            ...
            # NOTE: The run script remains in backend/scripts/ directory, no longer copied to the simulated directory
            # When starting a simulation, simulation_runner starts with scripts/ directory run script
    ...
    def get_run_instructions(self, simulation_id: str) -> Dict[str, str]:
        ...
        return {
            ...
            "instructions": (
                f"1. Activate conda environment: conda activate mirollama\n"
                ...
                f"   - Run both in parallel: python {scripts_dir}/run_parallel_simulation.py --config {config_path}"
            )
        }
