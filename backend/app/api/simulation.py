...
logger = get_logger('mirollama.api.simulation')


# Interview prompt Optimize prefix
# Adding this prefix can avoid the Agent calling the tool and reply directly with text.
INTERVIEW_PROMPT_PREFIX = "Based on your persona, memories, and actions, reply directly in plain text without using any tools: "
    ...
    Request (JSON):
        {
            ...
            "graph_id": "mirollama_xxxx",    // Optional, if not provided, it will be obtained from the project
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    # List of required files (excluding scripts, which are located in backend/scripts/)
    required_files = [
        "state.json",
    ...
    try:
        ...
        if status in prepared_statuses and config_generated:
            ...
            # If the status is preparing but the file has been completed, the automatically updated status isready
            if status == "preparing":
                try:
                    ...
                    logger.info(f"Auto-updated simulation state: {simulation_id} preparing -> ready")
                    status = "ready"
                except Exception as e:
                    logger.warning(f"Failed to auto-update state: {e}")
            ...
            return False, {
                "reason": f"Simulation is not prepared yet: status={status}, config_generated={config_generated}",
                "status": status,
                "config_generated": config_generated
            }
    ...
    Request (JSON):
        {
            ...
            "use_llm_for_profiles": true,                 // Optional, whether to use LLM to generate the character
            "parallel_profile_count": 5,                  // Optional, the number of characters generated in parallel, default5
    ...
    return:
        {
            ...
            "data": {
                ...
                "message": "Preparation task started | existing prepared result found",
...
def get_prepare_status():
    ...
    Request (JSON):
        {
            "task_id": "task_xxxx",          // Optional, returned by preparetask_id
            "simulation_id": "sim_xxxx"      // Optional, mock ID (used to check completed preparation)
        }
    ...
    try:
        ...
        if file_exists:
            ...
            try:
                ...
                logger.warning(f"Failed to read profiles file (it may still be writing): {e}")
                profiles = []
    ...
    try:
        ...
        if file_exists:
            ...
            try:
                ...
                logger.warning(f"Failed to read config file (it may still be writing): {e}")
                config = None
...
def get_simulation_config(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to get simulation config: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def download_simulation_script(script_name: str):
    """
    Download the simulation run script file (generic script, located at backend/scripts/)
...
def generate_profiles():
    ...
    Request (JSON):
        {
            "graph_id": "mirollama_xxxx",     // Required
...
def start_simulation():
    ...
    Regarding the force parameter:
        ...
        - Cleaned content includes:run_state.json, actions.jsonl, simulation.log Wait
    ...
    try:
        ...
        # Graph memory update is disabled in local-first migration mode.
    ...
    Request (JSON):
        {
            ...
            "platform": "reddit",              // Optional, default platform (overridden by each platform)
...
def interview_all_agents():
    ...
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",            // Required, simulateID
            "prompt": "What's your overall opinion on this matter?",  // Required, interview questions (all agents use the same questions)
    ...
    Request (JSON):
        {
            ...
            "agent_id": 0,                 // Optional, only obtain the interview history of the Agent
            "limit": 100                   // Optional, return quantity, default100
        }
